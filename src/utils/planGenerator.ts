...
function extractOrGenerateUnits(
  ...
  // Extrage linii ce seamănă cu unități sau capitole din textul extras / introdus
  // Suportă: "Unit 1: ...", "Unitatea 1: ...", "Capitolul 1: ...", "1. Titlu", "Modulul 1: ...", "Theme 1: ..."
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (unitName.length >= 3 && !extracted.some((u) => u.title.toLowerCase() === unitName.toLowerCase())) {
        extracted.push({
          title: unitName.startsWith("Unit") || unitName.startsWith("Capitol") ? unitName : `${isEnglish ? "Unit" : "Unitatea"}: ${unitName}`,
          contents: `Conținuturi tematice și activități conform resurselor curriculare (${unitName})`,
          cs: "1.1, 1.2, 2.1, 3.1",
        });
      }
    }
    if (extracted.length >= 14) break;
  }
...
export function generatePedagogicalPlan(params: PlanGenerationParams): string {
  ...
  const hasImageWithoutText = suportSnippets.some((s) => s.toLowerCase().includes("imagine atașată")) && unitList.length <= 6;
  const imageNotice = hasImageWithoutText
    ? `> 💡 **Notă:** Documentul a fost generat utilizând structura curriculară standard. Pentru a adapta unitățile conform cuprinsului unui manual specific, puteți introduce sau lipi titlurile capitolelor în secțiunea „Cuprins manual / capitole”.\n\n`
    : "";
